...
describe(`UploadRouter`, () => {
    ...
    describe(`"POST:${addUploadPath}"`, () => {
        ...
        code of "${BAD_REQUEST}" if the upload param was missing.`, (done: void) => {
            callApi()
                .end((err: Error, res: Response) => {
                    pErr(err);
        ...
        it(`should return a JSON object with an error message of "${paramMissingError}" and a status
        code of "${BAD_REQUEST}" if the upload "apk_file" param was missing.`, (done: void) => {
            callApi()
                .attach('apk_file', './tsconfig.json')
                .end((err: Error, res: Response) => {
                    pErr(err);
        ...
        it(`should return a JSON object with an error message of "${paramMissingError}" and a status
        code of "${BAD_REQUEST}" if the upload "json_file" param was missing.`, (done: void) => {
            callApi()
                .attach('json_file', './tsconfig.json')
                .end((err: Error, res: Response) => {
                    pErr(err);
